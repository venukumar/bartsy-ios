...
+(NSString *)decryptRSA:(NSString *)cipherString key:(NSString *)key data:(NSData*)incomingData
{
    ...
    [[NSUserDefaults standardUserDefaults]setObject:decryptedData forKey:@"Symmet"];
...
+(NSString *)encryptRSA:(NSString *)plainTextString key:(NSString *)key
{
    ...
    NSString *publicKeyIdentifier = [NSString stringWithFormat:@"RecipientPublicKey"];
...
+(NSData *)digtSign:(NSString *)cipherString key:(NSString *)key data:(NSData*)incomingData
{
	...
	CC_SHA1_Init(&ctx);
	// Perform the hash.
	CC_SHA1_Update(&ctx, (void *)[incomingData bytes], [incomingData length]);
	// Finalize the output.
    ...
    OSStatus err = SecKeyRawSign(privateKey, 
    ...
    if(err) 
    {
        NSLog(@"SecKeyRawSign failed: %ld",err);
        return nil;
    }
    NSData *decryptedData = [NSData dataWithBytes:signedHashBytes length:signedHashBytesSize];
    if(privateKey) CFRelease(privateKey);
